...
public interface EmployeeRepository extends JpaRepository<EmployeeMaster, Long>{
	...
	@Query(value = "SELECT e.emp_id as emp_id, e.email as email, " +
             "concat(e.first_name, ' ', e.last_name) as employeeName, " +
             "e.date_of_birth as date_of_birth, d.designation_name as designation_name, " +
             "d.job_role " +
             "FROM employee_master e " +
             "INNER JOIN designation_master d ON e.designation_id = d.designation_id " +
             "ORDER BY e.emp_id DESC",nativeQuery = true)
	List<Map<String, Object>> findAllEmployees(String string);
	
	@Query(value = "SELECT e.emp_id as emp_id, e.email as email, " +
            "concat(e.first_name, ' ', e.last_name) as employeeName, " +
            "e.date_of_birth as date_of_birth, d.designation_name as designation_name, d.job_role " +
            "FROM employee_master e " +
            "INNER JOIN designation_master d ON e.designation_id = d.designation_id "+
            "WHERE e.first_name LIKE %:searchKey% OR e.last_name LIKE %:searchKey% OR e.email LIKE %:searchKey% OR d.job_role LIKE %:searchKey% " +
            "OR d.designation_name LIKE %:searchKey% OR e.emp_id LIKE %:searchKey% " +
            "ORDER BY e.emp_id DESC ",nativeQuery = true)
	Page<Map<String, Object>> getAllEmployee(Pageable pageable, String searchKey);
	...
	@Query(value = "SELECT d.designation_id as designation, e.role_id as roleId, e.phone_number as phoneNumber,e.emp_id as emp_id,e.email as email,e.first_name as first_name,e.last_name as last_name,e.date_of_birth as date_of_birth,e.gender as gender,e.hire_date as hire_date, "
			+ "d.designation_name as designation_name,d.job_role as jobRole,dm.dept_id as department,dm.dept_name as department_name,"
			...
			+ "INNER JOIN team_master t ON e.team_id = t.team_id INNER JOIN experience_master em ON e.exp_id = em.exp_id INNER JOIN skill_transaction st "
	...
	List<Map<String, Object>> getEmployeeByFullName(String name);
	 ...
	 @Query(value = "SELECT CONCAT(e.first_name, ' ', e.last_name) AS headofDepartment,e.emp_id as empId\r\n"
	 		+ "FROM employee_master e\r\n"
	 		+ "JOIN employee_master m ON e.emp_id = m.manager_id group by headofDepartment,empId", nativeQuery = true)
